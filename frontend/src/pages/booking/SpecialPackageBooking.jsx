...
const STYLES = `
  ...
  .bk-next-btn{display:inline-flex;align-items:center;gap:8px;padding:12px 28px;
  ...
  .bk-next-btn::after{content:'';position:absolute;inset:0;
    background:linear-gradient(135deg,rgba(201,168,76,.22),transparent);opacity:0;transition:.22s}
  .bk-next-btn:hover{box-shadow:0 4px 20px rgba(139,26,26,.32);transform:translateY(-1px)}
...
const pkgHasBestMen = (pkg) => {
  ...
  return pkg.items.some((item) => {
    ...
    return n.includes("best man") || n.includes("bestman") || n.includes("groomsman") || n.includes("groomsmen");
  });
};
...
const SpecialPackageBooking = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // ── Auto-select package from navigation state ──
        // When navigating from /services/special with "Book This Package",
        // location.state.preselectedPackageId is set.
        const preselectedId = location.state?.preselectedPackageId;
        if (preselectedId) {
          const found = pkgList.find((p) => p.id === preselectedId);
          if (found) {
            // Apply the same selection logic as handleSelectPkg
            const linked = found.linkedDancingPackageId
              ? dpList.find((d) => d.id === found.linkedDancingPackageId) || null
              : null;
            ...
            // Jump straight to Step 1 — skip the package-selection step
            setStep(1);
          }
        }
      } catch {
  ...
  const handleSelectPkg = (pkg) => {
    ...
    setGroomDress("");
    setExtraPerformers([]);
    if (!pkgHasBestMen(pkg)) setBestMen([]); else setBestMen([""]);
    if (!pkgHasPageBoys(pkg)) setPageBoys([]); else setPageBoys([""]);
  };
  ...
  const validateEvent = () => {
    ...
    if (!eventForm.contactNo.trim())   errs.contactNo    = "Contact number is required";
    setEventErrors(errs);
  ...
  const removeBestMan = (i) => setBestMen((p) => p.filter((_, idx) => idx !== i));
  const updateBestMan = (i, v) => setBestMen((p) => p.map((x, idx) => (idx === i ? v : x)));
  const addPageBoy    = () => setPageBoys((p) => [...p, ""]);
  const removePageBoy = (i) => setPageBoys((p) => p.filter((_, idx) => idx !== i));
  const updatePageBoy = (i, v) => setPageBoys((p) => p.map((x, idx) => (idx === i ? v : x)));

  /* ─── Extra performers ─── */
  const addExtraPerformer = (ptId) => {
    setExtraPerformers((p) => {
      const exists = p.find((x) => x.performerTypeId === ptId);
      if (exists) return p.map((x) => x.performerTypeId === ptId ? { ...x, quantity: x.quantity + 1 } : x);
      return [...p, { performerTypeId: ptId, quantity: 1 }];
    });
  };
  const updateExtraQty = (ptId, delta) => {
    setExtraPerformers((p) =>
      p.map((x) => x.performerTypeId === ptId ? { ...x, quantity: Math.max(0, x.quantity + delta) } : x)
  ...
  const estimatedTotal   = basePackagePrice + dancingPriceAdjustment + extraPerformersTotal;

  /* ─── Navigation ─── */
  const canNext = () => {
    if (step === 0) return !!selectedPkg;
    if (step === 2) return !!groomDress;
    if (step === 3) return !!selectedDancingPkg || !selectedPkg?.linkedDancingPackageId;
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (groomDress) dressSelections.push({ role: "GROOM", dressItemId: groomDress });
      bestMen.forEach((id) => { if (id) dressSelections.push({ role: "BEST_MAN", dressItemId: id }); });
      pageBoys.forEach((id) => { if (id) dressSelections.push({ role: "PAGE_BOY", dressItemId: id }); });
  ...
  /* ── Label helpers ── */
  const dressLabel = (id) => dressItems.find((d) => d.dressItemId === id)?.dressItemName || id;
  ...
  const dancingPriceTag = (dp) => {
    ...
    if (diff > 0)  return <span className="bk-dp-price-adj extra">+ Rs. {fmt(diff)}</span>;
    if (diff < 0)  return <span className="bk-dp-price-adj saving">− Rs. {fmt(Math.abs(diff))}</span>;
  ...
  if (loadingData)
    return (
      <>
        ...
        <div className="bk-root"><div className="bk-spinner" /></div>
      </>
    );
    ...
    <>
      ...
      <div className="bk-root">
        ...
        <section className="bk-hero">
          <div className="bk-hero-badge"><Crown size={12} /> New Booking</div>
          <h1 className="bk-hero-title">Book a <em>Special Package</em></h1>
        ...
        <div className="bk-body">
                  ...
                  {packages.map((pkg) => (
                    <div key={pkg.id}
                      ...
                      <div className="bk-pkg-select-ring">
                        {selectedPkg?.id === pkg.id && <Check size={12} color="#fff" />}
                      </div>
                      ...
                      <div className="bk-pkg-badges">
                        {pkg.weddingCoordinationIncluded && <span className="bk-badge coord"><Crown size={9} /> Coordination</span>}
                        {pkg.weddingPackagingIncluded    && <span className="bk-badge pack"><Sparkles size={9} /> Packaging</span>}
                        {pkg.linkedDancingPackageName    && <span className="bk-badge dance"><Music2 size={9} /> {pkg.linkedDancingPackageName}</span>}
                        {pkg.discountPercent > 0         && <span className="bk-badge disc">✦ {pkg.discountPercent}% Discount</span>}
                      </div>
                      {pkg.freeItems?.length > 0 && (
                        <div style={{ fontSize: ".72rem", color: "var(--muted)", marginTop: "4px" }}>
                          + {pkg.freeItems.length} free inclusion{pkg.freeItems.length > 1 ? "s" : ""}
                        </div>
                      )}
                    </div>
                  ))}
          ...
          {step === 1 && (
            <>
              ...
              <p className="bk-section-sub">
                Tell us about your venue, date and timing so we can plan everything perfectly.
              </p>

              {/* Show which package is selected — helpful when arriving from /services/special */}
              {selectedPkg && (
                <div className="bk-preselect-banner">
                  <Crown size={14} style={{ flexShrink: 0 }} />
                  <span>
                    Selected package: <strong>{selectedPkg.name}</strong> — Rs. {fmt(selectedPkg.finalPrice)}
                  </span>
                </div>
              )}
              ...
              <div className="bk-form-grid">
                <div className="bk-field">
                  <label className="bk-label"><MapPin size={12} /> Hotel Name <span className="req">*</span></label>
                  <input name="hotelName" className="bk-input" placeholder="e.g. Cinnamon Grand Colombo"
                    value={eventForm.hotelName} onChange={handleEventChange} />
                  {eventErrors.hotelName && <p className="bk-err">{eventErrors.hotelName}</p>}
                </div>

                <div className="bk-field">
                  <label className="bk-label"><MapPin size={12} /> Nearest City <span className="req">*</span></label>
                  <input name="nearestCity" className="bk-input" placeholder="e.g. Colombo"
                    value={eventForm.nearestCity} onChange={handleEventChange} />
                  {eventErrors.nearestCity && <p className="bk-err">{eventErrors.nearestCity}</p>}
                </div>

                <div className="bk-field">
                  <label className="bk-label"><Calendar size={12} /> Event Date <span className="req">*</span></label>
                  <input name="eventDate" type="date" className="bk-input"
                    min={new Date().toISOString().split("T")[0]}
                    value={eventForm.eventDate} onChange={handleEventChange} />
                  {eventErrors.eventDate && <p className="bk-err">{eventErrors.eventDate}</p>}
                </div>

                <div className="bk-field">
                  <label className="bk-label"><Phone size={12} /> Contact Number <span className="req">*</span></label>
                  <input name="contactNo" className="bk-input" placeholder="e.g. 0771234567"
                    value={eventForm.contactNo} onChange={handleEventChange} />
                  {phonePrefilled && (
                    <p className="bk-hint">
                      <Info size={11} /> Auto-filled from your profile — you can change it if needed.
                    </p>
                  )}
                  {eventErrors.contactNo && <p className="bk-err">{eventErrors.contactNo}</p>}
                </div>

                <div className="bk-field">
                  <label className="bk-label"><Clock size={12} /> Groom Arrival Time</label>
                  <input name="groomArrivalTime" type="time" className="bk-input"
                    value={eventForm.groomArrivalTime} onChange={handleEventChange} />
                </div>

                <div className="bk-field">
                  <label className="bk-label"><Clock size={12} /> Poruwa Start Time</label>
                  <input name="poruwaStartTime" type="time" className="bk-input"
                    value={eventForm.poruwaStartTime} onChange={handleEventChange} />
                </div>

                <div className="bk-field full">
                  <label className="bk-label"><FileText size={12} /> Special Notes</label>
                  <textarea name="specialNotes" className="bk-input bk-textarea"
                    placeholder="Any special requests or notes for the team..."
                    value={eventForm.specialNotes} onChange={handleEventChange} />
                </div>
              </div>
            </>
          )}
          ...
          {step === 2 && (
            <>
              ...
              <p className="bk-section-sub">
                Choose traditional attire for each role included in your selected package.
              </p>
              ...
              <div className="bk-role-section">
                <div className="bk-role-head">
                  <div className="bk-role-title">
                    <div className="bk-role-icon groom"><Crown size={14} /></div>
                    Groom <span style={{ color: "var(--crimson)", fontSize: ".75rem" }}>*</span>
                  </div>
                </div>
                <div className="bk-dress-row">
                  <select className="bk-dress-select" value={groomDress}
                    onChange={(e) => setGroomDress(e.target.value)}>
                    <option value="">— Select a dress item —</option>
                    {dressItems.map((d) => (
                      <option key={d.dressItemId} value={d.dressItemId}>{d.dressItemName}</option>
                    ))}
                  </select>
                </div>
                {!groomDress && <p className="bk-err">Please select a dress for the Groom</p>}
              </div>
              ...
              {showBestMen ? (
                <div className="bk-role-section">
                  <div className="bk-role-head">
                    <div className="bk-role-title">
                      <div className="bk-role-icon bman"><Users size={14} /></div>
                      Best Man / Groomsmen
                      <span style={{ fontSize: ".72rem", color: "var(--muted)", fontFamily: "'DM Sans',sans-serif", fontWeight: 400 }}>
                        &nbsp;(optional)
                      </span>
                    </div>
                    <button className="bk-add-btn" onClick={addBestMan}><Plus size={12} /> Add Best Man</button>
                  </div>
                  {bestMen.map((val, i) => (
                    <div key={i} className="bk-dress-row">
                      <select className="bk-dress-select" value={val} onChange={(e) => updateBestMan(i, e.target.value)}>
                        <option value="">— Select a dress item —</option>
                        {dressItems.map((d) => (
                          <option key={d.dressItemId} value={d.dressItemId}>{d.dressItemName}</option>
                        ))}
                      </select>
                      <button className="bk-remove-btn" onClick={() => removeBestMan(i)}><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="bk-role-disabled">
                  <AlertCircle size={15} style={{ flexShrink: 0, color: "var(--subtle)" }} />
                  <span><strong>Best Man / Groomsmen</strong> dress selection is not available — your selected package does not include Best Men.</span>
                </div>
              )}
              ...
              {showPageBoys ? (
                <div className="bk-role-section">
                  <div className="bk-role-head">
                    <div className="bk-role-title">
                      <div className="bk-role-icon pboy"><User size={14} /></div>
                      Page Boys
                      <span style={{ fontSize: ".72rem", color: "var(--muted)", fontFamily: "'DM Sans',sans-serif", fontWeight: 400 }}>
                        &nbsp;(optional)
                      </span>
                    </div>
                    <button className="bk-add-btn" onClick={addPageBoy}><Plus size={12} /> Add Page Boy</button>
                  </div>
                  {pageBoys.map((val, i) => (
                    <div key={i} className="bk-dress-row">
                      <select className="bk-dress-select" value={val} onChange={(e) => updatePageBoy(i, e.target.value)}>
                        <option value="">— Select a dress item —</option>
                        {dressItems.map((d) => (
                          <option key={d.dressItemId} value={d.dressItemId}>{d.dressItemName}</option>
                        ))}
                      </select>
                      <button className="bk-remove-btn" onClick={() => removePageBoy(i)}><Trash2 size={14} /></button>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="bk-role-disabled">
                  <AlertCircle size={15} style={{ flexShrink: 0, color: "var(--subtle)" }} />
                  <span><strong>Page Boys</strong> dress selection is not available — your selected package does not include Page Boys.</span>
                </div>
              )}
            </>
          )}
          ...
          {step === 3 && (
            <>
              ...
              <p className="bk-section-sub">
                Your selected package includes a default dancing group. You may keep it or choose a different one — price differences will be reflected.
              </p>

              {selectedPkg?.linkedDancingPackageName && (
                <div className="bk-info">
                  <Music2 size={14} style={{ flexShrink: 0, color: "var(--gold)", marginTop: "2px" }} />
                  <span>
                    Your package already includes <strong>{selectedPkg.linkedDancingPackageName}</strong> at no extra cost.
                    Choosing a different package below will add or subtract from your total.
                  </span>
                </div>
              )}
                ...
                {dancingPkgs.map((dp) => (
                  <div key={dp.id}
                    ...
                    {selectedPkg?.linkedDancingPackageId === dp.id && (
                      <div style={{ fontSize: ".62rem", color: "var(--gold)", letterSpacing: ".10em",
                    ...
                    <div className="bk-dp-price" style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: "4px" }}>
                      Rs. {fmt(dp.totalPrice)}
                      {dancingPriceTag(dp)}
                    </div>
                    {dp.description && <div className="bk-dp-desc">{dp.description}</div>}
                    {dp.includedPerformers?.map((p) => (
                      <span key={p.id} className="bk-performer-chip">
                        <span className="bk-performer-count">{p.quantity}</span>{p.name}
                      </span>
                    ))}
                  </div>
                ))}
              ...
              {performerTypes.length > 0 && (
                <div className="bk-extra-section">
                  <div className="bk-extra-title">
                    ...
                    <span style={{ fontWeight: 300, letterSpacing: ".04em", textTransform: "none", color: "var(--muted)", fontSize: ".72rem" }}>
                      &nbsp;— beyond the dancing package
                    </span>
                  </div>
                  {performerTypes.map((pt) => {
                    const existing = extraPerformers.find((x) => x.performerTypeId === pt.id);
                    return (
                      <div key={pt.id} className="bk-extra-row">
                        <span style={{ flex: 1, fontSize: ".85rem", color: "var(--text)" }}>{pt.name}</span>
                        <span className="bk-extra-price">Rs. {fmt(pt.pricePerUnit)} / each</span>
                        <div className="bk-qty-ctrl">
                          <button className="bk-qty-btn" onClick={() => existing ? updateExtraQty(pt.id, -1) : null}>−</button>
                          <span className="bk-qty-val">{existing?.quantity || 0}</span>
                          <button className="bk-qty-btn" onClick={() => addExtraPerformer(pt.id)}>+</button>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="bk-price-strip">
                <div className="bk-price-strip-item">
                  ...
                  <span className="bk-price-strip-val">Rs. {fmt(basePackagePrice)}</span>
                </div>
                <div className="bk-price-divider" />
                {dancingPriceAdjustment !== 0 && (
                  <>
                    <div className="bk-price-strip-item">
                      <span className="bk-price-strip-label">Dancing Group Adjustment</span>
                      <span className="bk-price-strip-val" style={{ color: dancingPriceAdjustment > 0 ? "var(--crimson)" : "#15803D" }}>
                        {dancingPriceAdjustment > 0 ? "+" : "−"} Rs. {fmt(Math.abs(dancingPriceAdjustment))}
                      </span>
                    </div>
                    <div className="bk-price-divider" />
                  </>
                ...
                {extraPerformersTotal > 0 && (
                  <>
                    <div className="bk-price-strip-item">
                      <span className="bk-price-strip-label">Extra Performers</span>
                      <span className="bk-price-strip-val" style={{ color: "var(--crimson)" }}>
                ...
                <div className="bk-price-strip-item">
                  <span className="bk-price-strip-label">Estimated Total(Without Transport)</span>
                  <span className="bk-price-strip-val total">Rs. {fmt(estimatedTotal)}</span>
                </div>
              </div>
            </>
          )}
          ...
          {step === 4 && (
            <>
              ...
              <p className="bk-section-sub">
                Please review all your details before submitting the booking request.
              </p>

              {/* Package */}
              <div className="bk-review-card">
                <div className="bk-review-head"><Sparkles size={12} /> Selected Package</div>
                <div className="bk-review-body">
                  ...
                  <div className="bk-review-row">
                    ...
                    <span className="bk-review-val" style={{ color: "var(--crimson)" }}>
              ...
              {/* Event */}
              <div className="bk-review-card">
                <div className="bk-review-head"><MapPin size={12} /> Event Details</div>
                <div className="bk-review-body">
                  {[
              ...
              {/* Dress */}
              <div className="bk-review-card">
                <div className="bk-review-head"><Shirt size={12} /> Dress Selections</div>
                <div className="bk-review-body">
                  {groomDress && (
                    <div className="bk-dress-review-item">
                      <span className="bk-badge coord" style={{ flexShrink: 0 }}>Groom</span>
                      <span style={{ fontSize: ".85rem" }}>{dressLabel(groomDress)}</span>
                    </div>
                  )}
                  {bestMen.filter(Boolean).map((id, i) => (
                    <div key={i} className="bk-dress-review-item">
                      <span className="bk-badge dance" style={{ flexShrink: 0 }}>Best Man {i + 1}</span>
                      <span style={{ fontSize: ".85rem" }}>{dressLabel(id)}</span>
                    </div>
                  ))}
                  {pageBoys.filter(Boolean).map((id, i) => (
                    <div key={i} className="bk-dress-review-item">
                      <span className="bk-badge pack" style={{ flexShrink: 0 }}>Page Boy {i + 1}</span>
                      <span style={{ fontSize: ".85rem" }}>{dressLabel(id)}</span>
                    </div>
                  ))}
                </div>
              ...
              {/* Dancing */}
              <div className="bk-review-card">
                <div className="bk-review-head"><Music2 size={12} /> Dancing Package</div>
                <div className="bk-review-body">
                  {selectedDancingPkg ? (
                    <>
                      <div className="bk-review-row">
                        <span className="bk-review-key">Package</span>
                        <span className="bk-review-val">{selectedDancingPkg.name}</span>
                      </div>
                      <div className="bk-review-row">
                        <span className="bk-review-key">Package Price</span>
                        <span className="bk-review-val">Rs. {fmt(selectedDancingPkg.totalPrice)}</span>
                      </div>
                      {dancingPriceAdjustment !== 0 && (
                        <div className="bk-review-row">
                          <span className="bk-review-key">{dancingPriceAdjustment > 0 ? "Additional Cost" : "Saving"}</span>
                          <span className="bk-review-val" style={{ color: dancingPriceAdjustment > 0 ? "var(--crimson)" : "#15803D" }}>
                            {dancingPriceAdjustment > 0 ? "+" : "−"} Rs. {fmt(Math.abs(dancingPriceAdjustment))}
                          </span>
                        </div>
                      )}
                      {selectedDancingPkg.description && (
                        <div style={{ marginTop: "10px" }}>
                          <div style={{ fontSize: ".63rem", fontWeight: 600, letterSpacing: ".12em", textTransform: "uppercase", color: "var(--subtle)", marginBottom: "6px" }}>
                            What's included
                          </div>
                          <div className="bk-dp-review-desc">{selectedDancingPkg.description}</div>
                        </div>
                      )}
                    </>
                  ) : (
                    <p style={{ color: "var(--subtle)", fontSize: ".82rem" }}>No dancing package selected.</p>
                  )}

                  {extraPerformers.length > 0 && (
                    <div style={{ marginTop: "16px" }}>
                      <div style={{ fontSize: ".63rem", fontWeight: 600, letterSpacing: ".12em", textTransform: "uppercase", color: "var(--subtle)", marginBottom: "8px" }}>
                        Extra Performers
                      </div>
                      {extraPerformers.map((ep) => {
                        ...
                        return (
                          <div key={ep.performerTypeId} className="bk-review-row">
                            <span className="bk-review-key">{ep.quantity} × {pt?.name}</span>
                            <span className="bk-review-val" style={{ color: "var(--crimson)" }}>
              ...
              <div className="bk-review-card">
                ...
                <div className="bk-review-body">
                  <div className="bk-review-row">
                    ...
                    <span className="bk-review-val">Rs. {fmt(basePackagePrice)}</span>
                  </div>
                  {dancingPriceAdjustment !== 0 && (
                    <div className="bk-review-row">
                      <span className="bk-review-key">Dancing Package Adjustment</span>
                      <span className="bk-review-val" style={{ color: dancingPriceAdjustment > 0 ? "var(--crimson)" : "#15803D" }}>
                        {dancingPriceAdjustment > 0 ? "+" : "−"} Rs. {fmt(Math.abs(dancingPriceAdjustment))}
                      </span>
                    </div>
                  ...
                  {extraPerformersTotal > 0 && (
                    <div className="bk-review-row">
                      <span className="bk-review-key">Extra Performers Total</span>
                      <span className="bk-review-val" style={{ color: "var(--crimson)" }}>
                        + Rs. {fmt(extraPerformersTotal)}
                      </span>
                    </div>
                  ...
                  <div className="bk-review-total-row">
                    ...
                    <span className="bk-review-total-val">Rs. {fmt(estimatedTotal)}</span>
                  </div>
                </div>
              </div>

              <div className="bk-info">
                ℹ️ &nbsp;After submission, our team will review your request and add the transport price.
          ...
          <div className="bk-nav">
            ...
            {step < 4 ? (
              <button className="bk-next-btn" onClick={handleNext} disabled={!canNext()}>
                Next <ChevronRight size={15} />
              </button>
            ) : (
              <button className="bk-next-btn" onClick={handleSubmit} disabled={submitting}>
                {submitting
