...
const CategoryManager = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (editingCategory) {
        await api.put(`/api/admin/categories/${editingCategory.categoryId}`, data);
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg, padding:"28px 28px 60px", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))", gap:12, marginBottom:22 }}>
        {[{ label:"Total Categories", value:total, color:T.tx }].map(({ label, value, color }) => (
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.78rem", minWidth:600 }}>
            ...
            <tbody>
              {currentPageCategories.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding:"40px 16px", textAlign:"center", color:T.muted, fontSize:"0.85rem" }}>
                  ...
                  >
                    <Td>
                      {!visibleIds[cat.categoryId] ? (
                        <button onClick={() => toggleIdVisibility(cat.categoryId)} style={{ color:T.gold, fontSize:"0.72rem", background:"none", border:"none", cursor:"pointer", textDecoration:"underline" }}>Show</button>
                      ) : (
                        <>
                          <button onClick={() => toggleIdVisibility(cat.categoryId)} style={{ color:T.gold, fontSize:"0.72rem", background:"none", border:"none", cursor:"pointer", textDecoration:"underline" }}>Hide</button>
                          <div style={{ marginTop:3, wordBreak:"break-all", color:T.muted, fontSize:"0.68rem", fontFamily:"monospace", maxWidth:140 }}>{cat.categoryId}</div>
                        </>
                      )}
                    </Td>
                    <Td><span style={{ fontWeight:500, color:T.tx }}>{cat.name}</span></Td>
                    <Td><span style={{ color:T.muted }}>{cat.description || "—"}</span></Td>
                    <Td>{cat.createdAt ? new Date(cat.createdAt).toLocaleDateString("en-GB", { day:"2-digit", month:"short", year:"numeric" }) : "—"}</Td>
                    <Td>
                      <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                        <ActionBtn title="Edit" bg="rgba(201,168,76,0.12)" color={T.gold} hoverBg="rgba(201,168,76,0.24)" onClick={() => openModal(cat)}><Edit2 size={13} /></ActionBtn>
                        <ActionBtn title="Delete" bg={T.redBg} color={T.red} hoverBg="rgba(139,26,26,0.15)" onClick={() => handleDelete(cat.categoryId)}><Trash2 size={13} /></ActionBtn>
                      </div>
                    </Td>
                  </tr>
                ))
        ...
        {filteredCategories.length > 0 && (
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 18px", borderTop:`1px solid ${T.bdr}`, background:"rgba(250,247,244,0.70)", flexWrap:"wrap", gap:8 }}>
            <span style={{ fontSize:"0.75rem", color:T.muted }}>
              Showing <strong style={{ color:T.tx }}>{indexOfFirst+1}–{Math.min(indexOfLast, filteredCategories.length)}</strong> of <strong style={{ color:T.tx }}>{filteredCategories.length}</strong> categories
            </span>
            <div style={{ display:"flex", gap:6, alignItems:"center" }}>
      ...
      {modalOpen && (
        <div style={{ position:"fixed", inset:0, background:"rgba(28,10,0,0.65)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:16 }}>
          <div style={{ background:T.surf, borderRadius:16, border:`1px solid ${T.bdr}`, boxShadow:"0 24px 64px rgba(28,10,0,0.22)", width:"100%", maxWidth:500, maxHeight:"90vh", overflowY:"auto" }}>
            {/* Modal header */}
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 22px", borderBottom:`1px solid ${T.bdr}`, position:"sticky", top:0, background:T.surf, zIndex:10 }}>
              <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"1.5rem", fontWeight:700, color:T.tx, margin:0, display:"flex", alignItems:"center", gap:8 }}>
                {editingCategory ? <><Edit2 size={18} color={T.gold} /> Edit Category</> : <><Plus size={18} color={T.gold} /> Add New Category</>}
              </h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} style={{ padding:"20px 22px", display:"flex", flexDirection:"column", gap:16 }}>
              <Field label="Category Name" required error={errors.name?.message}>
                <input type="text" {...register("name")} placeholder="e.g. Mul-Adum" style={inputStyle} />
              </Field>

              <Field label="Description" error={errors.description?.message}>
                <textarea rows={4} {...register("description")} placeholder="Describe the category (optional)..." style={{ ...inputStyle, resize:"none", lineHeight:1.5 }} />
              </Field>

              {/* Footer */}
              <div style={{ display:"flex", justifyContent:"flex-end", gap:10, paddingTop:14, borderTop:`1px solid ${T.bdr}`, marginTop:4 }}>
                <button type="button" onClick={() => setModalOpen(false)} style={{ padding:"9px 20px", borderRadius:8, border:`1px solid ${T.bdr}`, background:T.surf, color:T.muted, cursor:"pointer", fontSize:"0.82rem", fontWeight:500 }}>Cancel</button>
                <button
                  type="submit"
                  ...
                  style={{ padding:"9px 24px", borderRadius:8, border:"none", cursor:loading||isSubmitting?"not-allowed":"pointer", fontSize:"0.82rem", fontWeight:600, color:"#fff", display:"flex", alignItems:"center", gap:7,
                    background: loading||isSubmitting ? "#aaa" : editingCategory ? "linear-gradient(135deg,#C9A84C,#E2C56A)" : T.red,
                    boxShadow: loading||isSubmitting ? "none" : "0 2px 10px rgba(139,26,26,0.22)" }}
                >
                  {loading||isSubmitting ? <><Loader2 size={14} className="animate-spin" /> Saving…</> : editingCategory ? "Update Category" : "Create Category"}
                </button>
              </div>
            </form>
          </div>
