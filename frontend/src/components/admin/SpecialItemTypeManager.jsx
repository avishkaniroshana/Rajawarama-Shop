...
const SpecialItemTypeManager = () => {
  ...
  const fetchTypes = async () => {
    try {
      const res = await api.get("/api/admin/special-item-types");
      setTypes(res.data || []);
    } catch (err) {
      toastError("Failed to load special item types");
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await api.delete(`/api/admin/special-item-types/${id}`);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
        <div>
          ...
          <p className="text-gray-600 text-xm">
            Define additional billable items for use special packages
          </p>
          <br />
          <button
            onClick={() => openModal()}
          ...
          >
            <Plus size={20} /> Add New Special Item Type
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/30 w-full max-w-lg max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-500 scrollbar-track-gray-100">
            <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-md border-b border-gray-200 px-8 py-5 flex justify-between items-center">
              ...
              >
                <X size={20} className="text-gray-600 hover:text-black-900" />
              </button>
            </div>
            ...
            >
              <div>
                ...
                <input
                  ...
                  placeholder="add special item name here"
                />
                {errors.name && (
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-5 py-3.5 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200/50 outline-none transition-all"
                  placeholder="999 (unlimited if empty)"
                />
                {errors.maxAvailable && (
                  <p className="text-red-500 text-sm mt-1">
