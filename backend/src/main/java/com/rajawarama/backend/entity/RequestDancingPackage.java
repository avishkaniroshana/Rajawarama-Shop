...
@Entity
@Table(name = "request_dancing_packages")
@Getter
@Setter
@NoArgsConstructor
public class RequestDancingPackage {

    @Id
    @GeneratedValue(strategy = GenerationType.UUID)
    @Column(name = "dp_request_id", nullable = false, updatable = false)
    ...
    // ------------------------------- Which dancing package is being booked
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "dancing_package_id", nullable = false)
    private DancingGroupPackage dancingPackage;
    ...
    // ----------- Extra performers on top of the dancing package
    @OneToMany(mappedBy = "dancingRequest", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<DancingRequestExtraPerformer> extraPerformers = new ArrayList<>();
